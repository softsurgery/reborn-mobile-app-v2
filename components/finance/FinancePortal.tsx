...
export const FinancePortal = ({ className }: FinancePortalProps) => {
  ...
  const pointTransactions = (txData?.pages.flatMap((page) => page.data) || []).map(tx => ({ ...tx, currencyType: 'POINTS' }));
  const fundTransactions = (fundTxData?.pages.flatMap((page) => page.data) || []).map(tx => ({ ...tx, currencyType: 'FUNDS' }));
                  ...
                  <View className="flex-1">
                    ...
                    >
                      {item.metadata?.title
                        ...
                        : (item as any).description || (item.currencyType === 'FUNDS' ? 'Fund Transaction' : 'Point Transaction')}
                    </Text>
