...
import { ScrollView, View, ActivityIndicator, RefreshControl, NativeSyntheticEvent, NativeScrollEvent } from "react-native";
    ...
    <View className="flex-1 px-5 mb-2">
      ...
      <View className="flex flex-row justify-between items-center w-full pt-2">
        <View className="flex flex-row items-center w-full border-hidden rounded py-1">
          ...
          <Input placeholder="Search message..." placeholderTextColor="#9ca3af" className="flex-1" />
        </View>
      </View>

      {/* List of active people */}
      <View>
        <Text className="text-gray-800 dark:text-gray-200 text-sm">List of active people</Text>
        <View className="-mx-5">
          <ScrollView horizontal showsHorizontalScrollIndicator={false} className="my-5 px-4">
            {users.map((user) => (
              <UserBubble key={user.uid} className="mx-1.5" label={user.surname} uid={user.uid} gender={user.isMale} />
            ))}
          </ScrollView>
        </View>
      </View>
      ...
      <ScrollView 
        ...
        refreshControl={
          <RefreshControl refreshing={isFetchingUsers} onRefresh={refetchUsers}/>
        }
      >
