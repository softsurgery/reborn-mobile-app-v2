...
import { useTopUpFunds, useTopUpPoints } from "~/hooks/content/finance/useFinance";
...
export default function TopUpScreen() {
  ...
  const isPending = topUpFundsMutation.isPending || topUpPointsMutation.isPending;
  ...
  return (
    <StableSafeAreaView className="flex-1 bg-card">
      <ApplicationHeader
        ...
        title={t("top_up_balance", "Top Up Balance")}
        titleVariant="large"
        reverse
        shortcuts={[
          {
          ...
          >
            <TabsList className="w-full flex-row h-12">
              ...
              <TabsTrigger value="points" className="flex-1 h-full">
                <Text>{t("add_points", "Add Points")}</Text>
              </TabsTrigger>
            </TabsList>
          </Tabs>
